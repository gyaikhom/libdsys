...
/* Sequential implementation of the Gaussian Elimination. */
...
/* Parallel Version: Buffering without memory copy. */
void gaussian_par_nmcpy(void)
{
	...
	if (bc_rank != 0) {
		src_pl = bc_plist_create(1, bc_rank - 1);
		src = bc_src_create(src_pl, ntype, BC_ROLE_PIPE);
	}
	if (bc_rank != last) {
		sink_pl = bc_plist_create(1, bc_rank + 1);

		/*
		 * Buffer size should be atleast 2 buffer units.
		 * No memocy copy Pipeline Role used.
		 */
		sink = bc_sink_create(sink_pl, ntype, 2, BC_ROLE_PIPEN);
	}

	/* Eliminate all the preceding rows. */
	for (i = 0; i < bc_rank; i++) {
		/*
		 * If this is the last process, there is no output
		 * buffer, a sink is not required.
		 */
		if (bc_rank == last) {
			bc_get(src, &A[i][0], 1);
			for (j = i+1; j < dim; j++)
				A[bc_rank][j] = A[bc_rank][j] - 
					A[bc_rank][i]*A[i][j];
			A[bc_rank][dim] = A[bc_rank][dim] - 
				A[bc_rank][i] * A[i][y];
		} else {
			/* Get data directly to the output buffer. */
			bc_get(src, bc_vptr(sink, float), 1);
			for (j = i+1; j < dim; j++)
				A[bc_rank][j] = A[bc_rank][j] - 
					A[bc_rank][i]*(*(bc_vptr(sink, float) + j));
			A[bc_rank][dim] = A[bc_rank][dim] - 
				A[bc_rank][i] * *(bc_vptr(sink, float) + y);
		}
		A[bc_rank][i] = 0;

		/*
		 * Note that we received the preceding row directly into the
		 * output buffer, and used it in the elimination (above). Because
		 * the value was not changed, we can directly commit.
		 */
		if (bc_rank != last)
			bc_commit(sink);
	}
	...
	/* Destroy branching channels and process list. */
	if (bc_rank != 0) {
		bc_chan_destroy(src);
...
void gaussian_par_mcpy(void)
{
	...
	if (bc_rank != last) {
		sink_pl = bc_plist_create(1, bc_rank + 1);
		sink = bc_sink_create(sink_pl, bc_float, y+1, BC_ROLE_PIPE);
	}
